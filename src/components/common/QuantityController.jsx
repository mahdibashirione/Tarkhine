...
const QuantityController = ({ id, className }) => {
  ...
  useEffect(() => {
    ...
    setQuantity(selectItem.quantity);
  }, [cart]);

  return (
    <div
      className={`${className} flex items-center text-primary justify-between p-2 text-xl rounded-lg`}
    >
      <button
        onClick={(e) => dispatch(increment(id))}
      ...
      {quantity <= 1 ? (
        ...
        <button
          onClick={(e) => dispatch(dicrement(id))}
          className="active:scale-95 duration-200"
        >
          <FiMinus />
        </button>
